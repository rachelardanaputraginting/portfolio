...
export default function Index(props) {
    ...
    const { data: achievements, meta, links } = props.achievements;
    ...
    return (
        <>
            <Modal show={isOpen} onClose={setIsOpen} >
                ...
                <p className='px-3 text-sm py-1 bg-gradient-to-r from-yellow-500 to-fifth flex items-center gap-2 max-w-max rounded m-4'>Juara {data.ranking}</p>
            ...
            <Container>
                <Title title="Admin - Achievement" active={true} />
            </Container>

            <Container>
                <Link href={route('admin.achievements.create')} className='rounded px-2 py-1.5 text-sm bg-gradient-to-r from-sky-600 to-fifth mb-4 flex items-center gap-1 my-12 max-w-max text-third'><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4">
                ...
                <Table>
                    ...
                    <Table.Tbody>
                        {achievements.length > 0 ? <>
                            {achievements.map((experience, i) => (
                                <>
                                    <tr key={experience.id}>
                                        <Table.Td>{i + meta.from}</Table.Td>
                                        <Table.Td>{experience.ranking}</Table.Td>
                                        <Table.Td>{experience.title}</Table.Td>
                                        <Table.Td>{experience.year}</Table.Td>
                                        <Table.Td>{experience.location}</Table.Td>
                                        <Table.Td>{experience.division}</Table.Td>
                                        <Table.Td className='flex gap-2'>
                                            ...
                                            <Link href={route('admin.achievements.edit', experience.slug)} className='transition duration-300 ease-linear px-1.5 py-1.5 rounded hover:scale-125 bg-gradient-to-r from-yellow-500 to-fifth'><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4">
                                                <path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                                            </svg>
                                            </Link>
                                            <Link onClick={() => {
                                                ask({
                                                    url: route('admin.achievements.destroy', experience.slug),
                                                    method: "delete",
                                                    data: "Achievement",
